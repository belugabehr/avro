...
public class ReflectNestedObjectArrayTest {
  ...
  @Benchmark
  @OperationsPerInvocation(BasicState.BATCH_SIZE)
  public void decode(final Blackhole blackhole, final TestStateDecode state)
      throws Exception {
    final Decoder d = state.decoder;
    final ReflectDatumReader<BasicRecord[]> datumReader =
        new ReflectDatumReader<>(state.schema);
  ...
  public static class TestStateEncode extends BasicArrayState {
    ...
    public TestStateEncode() {
      ...
      final String jsonText =
          ReflectData.get().getSchema(ObjectArrayWrapper.class).toString();
  ...
  public static class TestStateDecode extends BasicArrayState {
    ...
    public TestStateDecode() {
      super(ARRAY_SIZE);
      final String jsonText =
          ReflectData.get().getSchema(BasicRecord[].class).toString();
      this.schema = new Schema.Parser().parse(jsonText);
    }
    ...
    public void doSetupTrial() throws IOException {
      ...
      ReflectDatumWriter<BasicRecord[]> writer =
          new ReflectDatumWriter<>(schema);

      for (int i = 0; i < getBatchSize(); i++) {
        final BasicRecord[] r =
            populateRecordArray(getRandom(), getArraySize());
    ...
    public void doSetupInvocation() throws Exception {
      this.decoder = DecoderFactory.get().validatingDecoder(schema,
          super.newDecoder(this.testData));
    }
  }
